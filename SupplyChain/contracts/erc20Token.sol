...
contract ERC20Token is ERC20Interface {
    ...
    string public name;         // Descriptive name, example: "Testing Example Token"
    uint8 public decimals;      // How many decimals to use when displaying amounts
    string public symbol;       // Short identifier for token (ie: FDT)
    ...
    constructor (uint256 _initialAmount, string memory _tokenName, uint8 _decimalUnits, string memory _tokenSymbol) {
        ...
        name = _tokenName;                          // Stores the token name (for display only)
        decimals = _decimalUnits;                   // Stores the number of decimals (used for display only
        symbol = _tokenSymbol;                      // Stores the token symbol (used for display only)
    }

    // Transfer tokens from msg.sender to a specified address
    function transfer(address _to, uint256 _value) public returns (bool success) {
        require(balances[msg.sender] >= _value, "Insufficient funds for desired transaction.");
    ...
    function transferFrom(address _from, address _to, uint256 _value) public returns (bool success) {
        ...
        require(balances[_from] >= _value && allowance >= _value, "Insufficient allowed funds for desired transaction.");
        balances[_to] += _value;
    ...
    // Return the allowed amount of tokens for a transference
